...
List<Object^>^ DataManager::FindNodes(DataType type, String^ line) {
    ...
    case T_LINK:
        for each (Link^ link in (this->listLinks)) {
            if (link->ISBN->Contains(line) || link->dis_code->Contains(line)) {
                list->Add(link);
            }
        }
        break;
    case T_BOOK:
        for each (Book^ book in (this->listBooks)) {
            if (book->ISBN->Contains(line) || book->title->Contains(line) || book->author->Contains(line)) {
                list->Add(book);
            }
        }
        break;
    case T_SUBJECT:
        for each (Subject^ subj in (this->listSubjects)) {
            if (subj->dis_code->Contains(line) || subj->name->Contains(line) || subj->description->Contains(line)) {
                list->Add(subj);
            }
        }
        break;
